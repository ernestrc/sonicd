...
class PrestoSourceSpec(_system: ActorSystem)
  ...
  val defaultCol = ColMeta("name", "_type")
  ...
  val defaultStats = StatementStats.apply("FINISHED", true, 10, 0, 0, 10, 10, 10, 10, 10, 10)
  ...
  def assertRequest(req: HttpRequest, query: String) = {
    ...
    assert(req._2.toRelative.toString().endsWith("/statement"))
  ...
  "PrestoSource" should {
    ...
    "if query results state is not finished, it should use nextUri to get the next queryResults" in {
      ...
      }
      {
        ...
        val cmd = expectMsgType[HttpRequestCommand]
        assert(cmd.request._2.toRelative.toString().endsWith("2"))
      ...
      }
      {
        expectQueryProgress(100, QueryProgress.Running, Some(1000), Some("splits"))
        val cmd = expectMsgType[HttpRequestCommand]
        assert(cmd.request._2.toRelative.toString().endsWith("3"))
      ...
      }
      {
        expectQueryProgress(100, QueryProgress.Running, Some(1000), Some("splits"))
        val cmd = expectMsgType[HttpRequestCommand]
        assert(cmd.request._2.toRelative.toString().endsWith("4"))
      ...
      }
      {
        ...
        val cmd = expectMsgType[HttpRequestCommand]
        assert(cmd.request._2.toRelative.toString().endsWith("5"))
      ...
      }
      {
        ...
        val cmd = expectMsgType[HttpRequestCommand]
        assert(cmd.request._2.toRelative.toString().endsWith("6"))
      }
    }
